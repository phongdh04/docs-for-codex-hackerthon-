...
3. **Định hướng viết Concept Note & Story Specs:**
   ...
   - Từ repo UI được chọn, BA tra cứu đúng tài liệu Design System tương ứng bằng cách gọi MCP tool **`get_design_system`** (truyền tham số tên repository, nhận về đặc tả design system để làm căn cứ biên soạn `concept_note.md` chuẩn xác và đồng bộ.
