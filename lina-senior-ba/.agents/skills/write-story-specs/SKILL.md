...
2. **Viết Nghiệp vụ & Giao diện:**
   ...
   - Viết `concept_note.md` làm đầu vào cho Designer: Đối chiếu với danh sách `repositories` có `hasUi: true` từ `epicContext` để xác định giao diện của story thuộc repo nào (VD: `web-client` dùng Angular hay `pwa-client` dùng React/Tailwind). Gọi MCP tool **`get_design_system`** để tra cứu đúng tài liệu Design System tương ứng để biên soạn concept note chuẩn xác.
